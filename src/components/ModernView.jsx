    ...
    <div className={`modern-layout ${theme === "light" ? "light" : "dark"}`}>
      ...
      <header className="modern-header glass">
        ...
        <div className="header-actions">
          ...
          {modelsLoading && <span className="chip subtle">Загружаем модели…</span>}
          ...
          {isStreaming ? (
            ...
            <span className="chip subtle">Готов отвечать</span>
          )}
        </div>
      </header>
      ...
      <div className="modern-grid">
            ...
            {sorted.map((chat) => (
              ...
              >
                <div className="chat-pill-header">
                  <span className="chat-pill-name">{chat.name || "Без имени"}</span>
                  <div className="chat-pill-actions">
                    ...
                    <button
                      ...
                      onClick={(e) => {
                        e.stopPropagation();
                        const next = prompt("Новое имя", chat.name || "");
                ...
                <div className="chat-pill-sub">
                  {chat.messages.length ? `${chat.messages.length} сообщений` : "Пусто"}
                </div>
              </div>
            ))}
          </div>
        ...
        <main className="modern-chat glass">
          <div className="modern-chat-header">
            <div>
              <div className="chat-title">{active?.name || "Новый чат"}</div>
              <div className="chat-sub">
                {selectedModel} • {isStreaming ? "Стримим…" : "Готов"}
              </div>
            </div>
          ...
          <div className="modern-messages">
            {!active || active.messages.length === 0 ? (
              <div className="modern-empty">
                ...
                <h3>Спроси что-нибудь</h3>
                <p>ChatMock отвечает стримом. Названия чатов генерируются автоматически.</p>
              </div>
            ) : (
              active.messages.map((msg) => <ModernMessage key={msg.id} {...msg} />)
            )}
          </div>
...
function ModernMessage({ sender, type, content, pending, error }) {
  ...
  return (
    <div className={`modern-bubble ${isUser ? "user" : "bot"} ${pending ? "pending" : ""}`}>
      ...
      {error && <span className="bubble-status error">Ошибка</span>}
    </div>
  );
}
